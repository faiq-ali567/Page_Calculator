...
import { activeElement, contains, getDocument } from "../../floating-ui-react/utils.js";
/**
 * Adds support for delay, since Floating UI's `useFocus` hook does not support it.
 */
export function useFocusWithDelay(context, props = {}) {
  const {
    onOpenChange,
    elements,
    open,
  ...
  const reference = React.useMemo(() => ({
    ...
    onBlur(event) {
      ...
      timeout.start(0, () => {
        ...
        if (contains(dataRef.current.floatingContext?.refs.floating.current, activeEl) || contains(elements.domReference, activeEl)) {
          return;
        }
